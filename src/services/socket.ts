import { BareBoard, ClientToServerEvents, FileActionData, RoomData, ServerToClientEvents, SnapshotData } from '../types';
import { getSceneVersion, isInitializedImageElement, newElementWith, reconcileElements } from '../other/excalidraw';
import { SocketId } from '@excalidraw/excalidraw/dist/types/excalidraw/types';
import { DBUserPartial, DBUserPartialType } from '../other/vars';
import { performanceConstants } from '../core/constants';
import { hasAccessToBoard } from '../other/permissions';
import { compressionUtils } from '../modules/functions';
...
export default class SocketServer {
	...
	private async saveSpecificBoard(boardId: string): Promise<void> {
		...
		const existingFile = await this.manager.files.getBoardFile(boardId);
		...
		if (existingFile) {
			try {
				...
				this.manager.prometheus.recordError('version_compare_error', 'socket');
		...
		const uploaded = await this.manager.files.uploadBoardFile(boardId, compressed, 'application/octet-stream');
	...
	private async loadBoardElements(boardId: string): Promise<RoomData['elements']> {
		const s3File = await this.manager.files.getBoardFile(boardId);
		if (!s3File) return [];
		...
		try {
			...
			this.manager.prometheus.recordError('board_load_error', 'socket');
	...
	public async getRoomData(room: string): Promise<RoomData | null> {
		...
		const DBBoard = await db(this.manager, 'board', 'findUnique', { where: { boardId: room }, select: { files: true, boardId: true } });
		...
		try {
			...
			this.manager.prometheus.recordError('room_creation_error', 'socket');
	...
	private handleConnection(): void {
		this.io.on('connection', async (socket) => {
			this.manager.prometheus.updateSocketMetrics(this.io.sockets.sockets.size, this.roomData.size);

			const token = socket.handshake.auth.token as string;
			const targetRoom = socket.handshake.auth.room as string;
			if (!token || !targetRoom) return socket.disconnect(true);

			const DBUser = await db(this.manager, 'user', 'findFirst', { where: { email: token }, ...DBUserPartial });
			if (!DBUser) return socket.disconnect(true);

			const DBBoard = await db(this.manager, 'board', 'findUnique', { where: { boardId: targetRoom }, select: { files: true, boardId: true, version: true } });
	...
	private async setupSocket(socket: Socket<ClientToServerEvents, ServerToClientEvents>, DBBoard: BareBoard, DBUser: DBUserPartialType): Promise<void> {
		...
		socket.on('disconnect', async () => {
			...
			this.manager.prometheus.updateSocketMetrics(this.io.sockets.sockets.size - 1, this.roomData.size);
			...
			if (updatedRoom.collaborators.size) this.roomData.set(DBBoard.boardId, updatedRoom);
				...
				if (queued) {
					if (queued.length) await this.manager.files.deleteMediaFiles(DBBoard.boardId, queued);
					await this.manager.files.deleteUnusedFiles(DBBoard.boardId);
		...
		socket.on('userFollow', async (data) => {
			this.manager.prometheus.recordSocketEvent('userFollow');
		...
		socket.on('sendSnapshot', (data) => {
			if (!DBBoard.canEdit) return;
			this.manager.prometheus.recordSocketEvent('sendSnapshot');
